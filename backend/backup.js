...
const resolveVerbosityConstraints = (verbosityLevel, routing, baseMaxTokens) => {
    let verbosityVal = verbosityLevel;
    if (typeof verbosityVal === 'string') {
        if (verbosityVal === 'short') verbosityVal = 50;
        else if (verbosityVal === 'detailed') verbosityVal = 350;
        else verbosityVal = 150;
    }

    if (verbosityVal >= 1000) { // Max slider value = No constraints
        return {
            instruction: "\n\n[CRITICAL REQUIREMENT: Provide a highly detailed, comprehensive analysis. You have NO length constraints. Take as much space as you need to complete all thoughts naturally.]",
            maxTokens: routing?.maxTokensDetailed || Math.max(baseMaxTokens, 4000)
        };
    } else if (verbosityVal <= 100) {
        return {
            instruction: `\n\n[ABSOLUTE MANDATORY CONSTRAINT: Generate EXACTLY 1 to 2 short sentences. You MUST NOT exceed ${verbosityVal} words under any circumstances. Wrap up your thought immediately to ensure you do not get cut off.]`,
            maxTokens: routing?.maxTokensShort || Math.max(baseMaxTokens, 500)
        };
    } else {
        return {
            instruction: `\n\n[ABSOLUTE MANDATORY CONSTRAINT: Provide your detailed analysis, but you MUST NOT exceed ${verbosityVal} words. You must pace your output and write a final, natural concluding paragraph well before reaching this limit. Going over ${verbosityVal} words is strictly forbidden, so prioritize wrapping up your thoughts completely.]`,
            maxTokens: Math.max(routing?.maxTokensMedium || 1000, Math.floor(verbosityVal * 1.5), baseMaxTokens)
        };
    }
};
...
router.post('/generate/:targetId', async (req, res) => {
    try {
        ...
        if (routing) {
            ...
            const resolved = resolveVerbosityConstraints(verbosityLevel, routing, dynamicMaxTokens);
            verbosityInstruction = resolved.instruction;
            dynamicMaxTokens = resolved.maxTokens;
        }
...
router.post('/chat/:targetId', async (req, res) => {
    try {
        ...
        systemInstruction = `${goldenRulesStr}\n${systemInstruction}\n${verbosityInstruction}`;
