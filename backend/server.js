...
const startVoiceEngine = () => {
    ...
    const enginePath = path.join(__dirname, "voice_engine", "server.py");
    if (!fs.existsSync(enginePath)) {
        // Silently abort if the python voice engine doesn't exist
        return;
    }

    console.log("🎙️ Starting Python Voice Engine...");
    
    // Check if python or python3 is available
    const pythonCmd = process.platform === "win32" ? "python" : "python3";
    
    voiceEngineProcess = spawn(pythonCmd, [enginePath], {
        stdio: "inherit",
        shell: true
    });

    voiceEngineProcess.on("close", (code) => {
        console.log(`🎙️ Voice Engine exited with code ${code}`);
        if (!isShuttingDown) {
            console.log("🎙️ Restarting Voice Engine in 3 seconds...");
            setTimeout(startVoiceEngine, 3000);
        }
    });
};
