...
export function DayPanel({ date, dayData, onClose }) {
  ...
  const isOffDay     = state === 'weekend' || state === 'holiday';
    ...
    <div className="h-full flex flex-col bg-background-surface">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border-default bg-background-card shrink-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          {/* Icon badge */}
          <div className={`p-2.5 rounded-xl border ${isOffDay ? 'bg-amber-500/10 border-amber-500/20' : 'bg-blue-500/10 border-blue-500/20'}`}>
            <Calendar className={`w-6 h-6 ${isOffDay ? 'text-amber-500' : 'text-blue-500'}`} />
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              {isOffDay ? (
                <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-500 border border-amber-500/20 uppercase tracking-wider">
                  {state === 'holiday' ? 'Market Holiday' : 'Weekend'}
                </span>
              ) : (
                <>
