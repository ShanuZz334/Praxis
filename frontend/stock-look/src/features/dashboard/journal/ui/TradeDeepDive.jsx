            ...
            <div className="bg-[var(--bg-tooltip)] border border-border-default rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto invisibleScroll shadow-2xl relative">
                ...
                <div className="px-6 py-5 border-b border-border-default flex items-start justify-between bg-background-surface">
                    <div>
                        ...
                        <h2 className="text-2xl font-bold text-text-primary tracking-tight">{trade.instrument} <span className="text-text-tertiary text-lg">({trade.direction})</span></h2>
                        <div className="text-xs font-bold text-text-tertiary uppercase mt-1">Strategy: {trade.strategy}</div>
                    </div>
                    ...
                    <Section title="Decision Context" icon={<Target size={14} />}>
                        <div className="grid grid-cols-2 gap-6">
                            <div className="bg-background-surface p-4 rounded-lg border border-border-subtle">
                                ...
                                <div className="text-lg font-bold text-text-primary mb-1">{trade.psychology.state}</div>
                                <div className="text-xs text-text-secondary italic">"{trade.psychology.notes}"</div>
                            </div>
                            <div className="bg-background-surface p-4 rounded-lg border border-border-subtle space-y-2">
                                <div className="flex justify-between">
                                    <span className="text-xs text-text-tertiary font-bold uppercase">Market Regime</span>
                                    <span className="text-xs text-text-primary font-mono">{trade.context.regime}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-xs text-text-tertiary font-bold uppercase">Volatility</span>
                                    <span className="text-xs text-text-primary font-mono">{trade.context.vol}</span>
                                </div>
                            </div>
                        </div>
                    </Section>
