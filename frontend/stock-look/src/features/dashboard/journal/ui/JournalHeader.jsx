...
export default function JournalHeader({ capital, score }) {
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <KPICard label="Account Equity" value={`₹${capital.capital.toLocaleString()}`} sub="High Water Mark" subValue="98%" accent="text-slate-100" />
...
function KPICard({ label, value, sub, subValue, accent, isGrade }) {
    return (
        <div className="relative bg-[#0b1220] border border-white/5 rounded-2xl p-4 shadow-[0_8px_24px_rgba(0,0,0,0.45)] hover:border-white/10 transition-colors flex flex-col justify-between min-h-[100px] group overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col justify-between h-full">
                ...
                <div>
                    ...
                    <div className="flex justify-between items-center mt-1 pt-2 border-t border-white/5">
                        <span className="text-[9px] text-slate-600 uppercase font-semibold">{sub}</span>
                        <span className="text-[9px] text-slate-400 font-mono">{subValue}</span>
                    </div>
                </div>
            </div>
        </div>
