...
export default function EPSYoYCard({ cardId, data, manualOverride, lastUpdated }) {
    ...
    const configData = getIndicatorConfig(CARD_REGISTRY.eps_yoy.id);

    // 3. Praxis Engine
    // Note: Most macro indicators just take a single value for scoring
    const scoreObj = scoreNiftyEPSGrowth(currentValue);
    const { score, bias, confidence, trendDesc } = scoreObj;
    const aiInsightText = generateAiInsightNiftyEPSGrowth(scoreObj, currentValue);
    ...
    return (
        <IndicatorCard
            ...
            insights={{
                ...
                whyItMatters: ["Tracks aggregate earnings growth.","Key driver for index performance."]
            }}
        />
    );
}
