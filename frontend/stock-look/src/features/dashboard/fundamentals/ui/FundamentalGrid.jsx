...
export default function FundamentalGrid({ cards, viewMode, sortMode = "score_desc", onCardClick, data, selectedCategory, manualOverrides, resolveTime }) {
  ...
  const getHardcodedNode = (cardId) => {
    switch(cardId) {
        ...
        case CARD_REGISTRY.fii_dii_flow.id: return <FIIDIIFlowCard key={cardId} cardId={cardId} data={data} manualOverride={manualOverrides?.fii_dii_flow} lastUpdated={(isLive) => resolveTime(isLive, isLive ? null : CARD_REGISTRY.fii_dii_flow.id)} />;
