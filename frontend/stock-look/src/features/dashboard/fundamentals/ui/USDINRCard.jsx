...
import { scoreUSDINR, generateAiInsightUSDINR } from '@/features/dashboard/fundamentals/engine/scoringEngine';

export default function USDINRCard({ data, manualOverride, lastUpdated }) {
    ...
    const configData = getIndicatorConfig('usdinr');

    // 3. Praxis Engine
    // Note: Most macro indicators just take a single value for scoring
    const scoreObj = scoreUSDINR(currentValue);
    const { score, bias, confidence, trendDesc } = scoreObj;
    const aiInsightText = generateAiInsightUSDINR(scoreObj, currentValue);
    ...
    return (
        <IndicatorCard
            ...
            insights={{
                ...
                whyItMatters: ["Measures currency strength.","Depreciation increases imported inflation."]
            }}
        />
    );
}
