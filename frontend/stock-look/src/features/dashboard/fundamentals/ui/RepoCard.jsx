...
import { scoreRepoRate, generateAiInsightRepoRate } from '@/features/dashboard/fundamentals/engine/scoringEngine';

export default function RepoCard({ cardId, data, manualOverride, lastUpdated }) {
    ...
    const configData = getIndicatorConfig(CARD_REGISTRY.repo.id);

    // 3. Praxis Engine
    // Note: Most macro indicators just take a single value for scoring
    const scoreObj = scoreRepoRate(currentValue);
    const { score, bias, confidence, trendDesc } = scoreObj;
    const aiInsightText = generateAiInsightRepoRate(scoreObj, currentValue);
    ...
    return (
        <IndicatorCard
            ...
            insights={{
                ...
                whyItMatters: ["Central bank policy rate.","Lower rates boost equity valuations."]
            }}
        />
    );
}
