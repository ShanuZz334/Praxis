...
function parseHeadlessFundamentals(rawFundamentals) {
    const scores = {};
    const cards = [];
    ...
    const attemptComputation = (id) => {
        if (!rawFundamentals) return { success: false, reason: "No upstream data" };
...
export class FundamentalEngine {
    ...
    parse(rawData) {
        ...
        this.cache = parseHeadlessFundamentals(rawData);
    }
