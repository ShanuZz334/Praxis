...
let startIndex = content.indexOf('const priceAnalytics =');
let endIndex = content.indexOf('${paeReport}');

if (startIndex !== -1 && endIndex !== -1) {
    let before = content.substring(0, startIndex);
    let after = content.substring(endIndex);
    
    let replacement = `const priceAnalytics = _computePriceAnalytics(window50, window20, window5, last, prev, indicators);
